...
const page = () => {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    try {
      const url = subject_id === 0 ? `http://localhost:3000/api/university/${uni_id}/subject` :
  ...
  const get_sub_data = subject.map((data, index) => {
    return (
      <div className='main_content group shadow-md relative justify-center items-center w-full font-bold rounded-sm' key={index}>
        ...
        <button onClick={(e) => { handle_edit(data.subject_name); setSubject_id(data.id) }} className='flex gap-1 hover:text-green-600 border-r border-black p-2'><FiEdit size={20} /><span>Edit</span></button>
