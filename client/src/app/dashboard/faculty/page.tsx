...
const page = () => {
  ...
  useEffect(()=>{
    ...
    const getFaculty = async (id:any) => {    
      const response = await fetch(`http://localhost:3000/api/university/${id}/faculty`);
    ...
    const handle_delete = async (fac_id:any) => {
      const response = await fetch(`http://localhost:3000/api/university/${uni_id}/faculty/${fac_id}`, {
        method: 'DELETE',
      });
    ...
    const handleSubmit = async (e:any) => {
      e.preventDefault();
      try {
        const url = faculty_id == '' ? `http://localhost:3000/api/university/${uni_id}/faculty` :
        `http://localhost:3000/api/university/${uni_id}/faculty/${faculty_id}`;
  
        const method = faculty_id == '' ? 'POST' : 'PUT';
    ...
    const get_fac_data = faculty.map((data) =>{
      return (
          <div className='shadow-md hover:bg-slate-100 flex flex-col justify-center items-center w-full p-5 gap-0 font-bold rounded-sm'>
            ...
            <div className='flex gap-1 mt-5'>
              <button onClick={(e) => {handle_edit(data.faculty_name); setFaculty_id(`${data.id}`)}} className='bg-green-600 px-3 py-1 rounded-md'><LuPencil size={20} className=' text-white '></LuPencil></button>
    ...
    <div className='flex flex-col gap-6 justify-center items-center p-5 w-full'>
      <div className='flex justify-between w-full'>
        ...
        <div className="grid grid-cols-3 w-full gap-5">
          {faculty.length > 0 ? get_fac_data : <div></div>}
        </div>
    </div>
  )
}
