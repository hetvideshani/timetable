...
const page = () => {
  ...
  const getSessions = async (id: any) => {
    const response = await fetch(`http://localhost:3000/api/university/${id}/session`);
  ...
  const handle_edit = (ses: any) => {
    setIsModalOpen(true);
    setSession({ ...session, dept_id:ses.dept_id, id:ses.id, do_nothing:ses.do_nothing, duration: ses.duration, session_sequence: ses.session_sequence });
  }
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const url = session.id === null ? `http://localhost:3000/api/university/${uni_id}/department/${session.dept_id}/session` :
      ...
      if (result.function_name === 'update_session') {
        setAllSessions((prev: any) => prev.map((ses: any) => ses.id === session.id ? { ...ses,  dept_id:ses.dept_id, do_nothing:ses.do_nothing, duration: ses.duration, session_sequence: ses.session_sequence } :ses));
      }
      ...
      setSession({ id: null, dept_id : null, do_nothing: false, duration: null, session_sequence: null });
      router.refresh();
    } catch (error) {
  ...
  const handle_delete = async (br: any) => {
    ...
    if (data.status === 201) {
      ...
      setSession({ id: null, session_sequence: null, dept_id : null, do_nothing: false, duration: null });
      router.refresh();
    } else {
  ...
  const get_session_data = all_sessions.map((data, index) => {
    return (
      <div className='shadow-md hover:bg-slate-100 flex flex-col justify-center items-center w-full p-5 gap-0 font-bold rounded-sm' key={index} onClick={() => setSession({ id: data.id, session_sequence: data.session_sequence, duration: data.duration, dept_id: data.dept_id, do_nothing: data.do_nothing })}>
            ...
            <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
              ...
              <form onSubmit={handleSubmit}>
                <input
                  id="data"
                  type="number"
                  value={session.session_sequence ? (session.session_sequence) : ""}
                  placeholder='Enter Session Sequence'
                  onChange={(e) => setSession({ ...session, session_sequence: Number(e.target.value) })}
                ...
                <input
                  ...
                  onChange={(e) => setSession({ ...session, duration: Number(e.target.value) })}
                  className="mt-1 p-2 border border-gray-300 rounded-md w-full"
                />
                ...
                <label>Is Break Session ?</label>
                  <input
                    ...
                    onChange={(e) => setSession({ ...session, do_nothing: e.target.checked })}
                  />
                </div>
